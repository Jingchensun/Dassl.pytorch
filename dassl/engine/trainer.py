...
class SimpleTrainer(TrainerBase):
    ...
    def after_epoch(self):
        ...
        do_test = not self.cfg.TEST.NO_TEST
        # print("do_testdo_testdo_testdo_test:", do_test) #True
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            image, label, text = self.parse_batch_test(batch)
            # print('image:', image.size())
            # print('label:', label.size())
            # print('text:', text.size())
            cache_feature = None
            input_u = None
            # output = self.model_inference(image, label)
            output, image_feature = self.model_inference(image, label, text, cache_feature, input_u)
            self.evaluator.process(output, label)
        #     feature = image_feature.cpu()
        #     for idx in range(len(image_feature )):
        #         feature_list.append(feature[idx].tolist())
        #     label_list.extend(label.tolist())
        # save_dir = '/home/jingchen/promtsrc/lpclip/prompt_feat_test/'
    ...
    def model_inference(self, input, label, text, cache_feature, input_u):
        return self.model(input, label, text, cache_feature, input_u)
